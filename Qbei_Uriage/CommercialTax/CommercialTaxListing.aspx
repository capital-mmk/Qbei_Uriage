<%@ Page Title="" Language="C#" MasterPageFile="~/Uriage.Master" AutoEventWireup="true" CodeBehind="CommercialTaxListing.aspx.cs" Inherits="Qbei_Uriage.CommercialTax.CommercialTaxListing" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#<%=txtCUPrice.ClientID%>").on("keypress keyup", function (e) {
                $(this).val($(this).val().replace(/[^0-9\.]/g, ''));
                if (e.which < 48 || e.which > 57)
                    e.preventDefault();
            });

            $("#<%=txtCPer.ClientID%>").on("keypress keyup", function (e) {
                $(this).val($(this).val().replace(/[^0-9\.]/g, ''));
                if (e.which < 48 || e.which > 57)
                    e.preventDefault();
            });

            $("#<%=gvCommercial.ClientID%> [id*=btnDelete]").click(function () {
                ...
                if (confirm("削除よろしいですか？"))
                {
                    $.ajax({
                        ...
                        url: "CommercialTaxListing.aspx/CommercialTaxDelete",
                        ...
                        success: function (msg) {
                            ...
                            {
                                temp.remove();
                                if ($("#<%=gvCommercial.ClientID%> td").length == 0)
                                { }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div class="panel panel-custom1" style="margin-top:-40px;">
        <div class="panel-heading" style="width:100%"><span class="glyphicon glyphicon-file">決済手数料一覧</span></div>
                ...
                <div class="panel-body" style="padding: 10px 0px 0px 0px; margin-left:10px; margin-top:10px;">
                    <table style="height: 80px; width: 1328px;">
                        <tr>
                            <td>勘定科目</td>
                            <td>
                                <asp:TextBox ID="txtCAcc" runat="server" Width="130px" Height="25px"></asp:TextBox>
                            </td>
                            <td>支払区分コード</td>
                            <td>
                                <asp:TextBox ID="txtPayment" runat="server" Width="130px" Height ="25px"></asp:TextBox>
                            </td>
                            <td>単価</td>
                            <td>
                                <asp:TextBox ID="txtCUPrice" runat ="server" Width ="130px" Height ="25px"></asp:TextBox>&nbsp;円
                            </td>
                        </tr>
                        <tr>
                            <td>割合</td>
                            <td>
                                <asp:TextBox ID="txtCPer" runat="server" Width="50px" Height="25px"></asp:TextBox>&nbsp;%
                            </td>
    ...
    <div align="center">
        <asp:GridView ID="gvCommercial" runat="server" AllowPaging ="true" AutoGenerateColumns="false" Width="98%"
            ShowHeaderWhenEmpty ="true" CssClass ="table table-striped table-bordered table-hover usd header-m" OnPageIndexChanging="gvCommercial_PageIndexChanging"
            ...
            <Columns>
                <asp:TemplateField>
                    ...
                    <ItemTemplate>
                        <asp:HiddenField runat="server" ID ="hdCommercialId" Value ='<%#Bind("CommercialTaxID") %>' />
                        <asp:Label runat="server" ID="lblCAcc" Text ='<%#Bind("AccountTitle") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
                    <HeaderStyle CssClass="text-center" />
                    <HeaderTemplate>支払区分コード</HeaderTemplate>
                    <ItemTemplate>
                        <asp:Label runat="server" ID="lblPaymentCd" Text='<%#Bind("ShopSectionCode") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
                    <HeaderStyle CssClass="text-center" />
                    ...
                    <ItemTemplate>
                        <asp:Label runat="server" ID ="lblCUnitPrice" Text='<%# (int.Parse (Eval("UnitPrice").ToString() )> 0) ? int.Parse ( Eval("UnitPrice").ToString()).ToString("#,###") + " 円"  : string.Empty%>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
                    <HeaderStyle CssClass="text-center" />
                    ...
                    <ItemTemplate>
                        <asp:Label runat="server" ID="lblCPercent" Text ='<%# (decimal.Parse (Eval("Percent").ToString()) > 0)? (decimal.Parse( Eval("Percent").ToString()) * 100).ToString("###") + " %" : string.Empty %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
                    <HeaderStyle CssClass="text-center" />
                    ...
                    <ItemTemplate>
                        <asp:Label runat="server" ID ="lblCStartDT" Text ='<%#Bind("Expire_SDate","{0:yyyy/MM/dd}") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
                    <HeaderStyle CssClass="text-center" />
                    ...
                    <ItemTemplate>
                        <asp:Label runat="server" ID="lblCEndDT" Text='<%#Bind("Expire_EDate","{0:yyyy/MM/dd}") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
                    ...
                    <ItemTemplate>
                        <a id ="btnEdit" runat="server" class="btn btn-info btn-xs" onserverclick="btnEdit_Click">
                            <span aria-hidden="true" class="glyphicon glyphicon-edit">
                                <asp:Label runat="server" Text="編集"></asp:Label>
                            </span>
                        </a>
                    </ItemTemplate>
                </asp:TemplateField>
