...
namespace Qbei_Uriage.UserList
{
    public partial class User_Entry : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    change_name.InnerText = "パスワード";
                    //txtpassword.Attributes["placeholder"] = "Enter your Password";
                }
                //else {
        ...
        protected void Edit()
        {
            string[] str = Session["UserInfo"] as string[];
            ...
            txtUserName.Value = str[1];
            txtModifiedDate.Value = "";


            txtpassword.Value = str[2];
            txtpassword.Attributes["type"] = "password";
            try
            {
            ...
            txtpassword.Attributes.Add("value", str[2]);
        ...
        protected void Save()
        {
            ...
            if (!String.IsNullOrWhiteSpace(txtUserName.Value) && !String.IsNullOrWhiteSpace(txtpassword.Value))
            {
                ...
                if (dt.Rows.Count == 0)
                {
                    ...
                    {
                        Response.Write("<script>alert('登録失敗しました');</script>");
                    }
                }
                else
                {
                    Response.Write("<script>alert('ユーザーは既に存在しています');</script>");
                }
            }
            else
            {
                Response.Write("<script>alert('ユーザー名とパスワードを入力してください');</script>");
            }
        }
        protected void Update()
        {
            ...
            que.UserName = txtUserName.Value;
            que.Password = txtpassword.Value;
            que.ModifiedDate = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss");
            que.Modified_By = str[1];
            if (!String.IsNullOrEmpty(que.UserName) && !String.IsNullOrEmpty(txtpassword.Value))
            {
                ...
                {
                    Response.Write("<script>alert('更新失敗しました');</script>");
                    string url_id = Request.Url.AbsoluteUri;
                    DataTable dt = new DataTable();
                    que.UserID = Request.QueryString["UserID"];
                    //que.UserName = "";
