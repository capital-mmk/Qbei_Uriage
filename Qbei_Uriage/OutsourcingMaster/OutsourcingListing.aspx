<%@ Page Title="" Language="C#" MasterPageFile="~/Uriage.Master" AutoEventWireup="true" CodeBehind="OutsourcingListing.aspx.cs" Inherits="Qbei_Uriage.OutsourcingMaster.OutsourcingListing" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script type ="text/javascript">
        $(document).ready(function () {
            $("#<%=txtUPrice.ClientID%>").on("keypress keyup", function (e) {
                $(this).val($(this).val().replace(/[^0-9\.]/g, ''));
                if (e.which < 48 || e.which > 57)
                    e.preventDefault();
            });

            $("#<%=gvOutsourcing.ClientID%> [id*=btnDelete]").click(function () {
                ...
                if (confirm("削除よろしいですか？"))
                {
                    $.ajax({
                        ...
                        url: "OutsourcingListing.aspx/OutsourcingDelete",
                        ...
                        success: function (msg) {
                            ...
                            {
                                temp.remove();
                                if ($("#<%=gvOutsourcing.ClientID%> td").length == 0)
                                { }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div class="panel panel-custom1" style="margin-top:-40px;">
        <div class="panel-heading" style="width:100%"><span class="glyphicon glyphicon-file">外注作業費一覧</span></div>
                ...
                <div class="panel-body" style="padding: 10px 0px 0px 0px; margin-left:10px; margin-top:10px;">
                    <table style="height: 80px; width: 1328px;">
                        <tr>
                            ...
                            <td>単価</td>
                            <td>
                                <asp:TextBox ID="txtUPrice" runat="server" Width="130px" Height="25px"></asp:TextBox> 円
                            </td>
                            <td>開始日付</td>
                            <td>
    ...
    <div align="center">
        <asp:GridView runat="server" ID="gvOutsourcing" AllowPaging="true" AutoGenerateColumns="false" Width="98%"
            ShowHeaderWhenEmpty ="true" CssClass ="table table-striped table-bordered table-hover usd header-m" OnPageIndexChanging="gvOutsourcing_PageIndexChanging"
            ...
            <Columns>
                <asp:TemplateField>
                    ...
                    <ItemTemplate>
                        <asp:HiddenField runat="server" ID="hdOutsourcingId" Value ='<%#Bind("OutsourcingID") %>' />
                        <asp:Label runat="server" ID ="lblOAcc" Text = '<%#Bind ("AccountTitle")%>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
                    ...
                    <HeaderStyle  CssClass="text-center"/>
                    <ItemTemplate>
                        <asp:Label runat="server" ID ="lblUnitPrice" Text ='<%#(int.Parse (Eval("UnitPrice").ToString())).ToString("#,###") + " 円" %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
                    ...
                    <HeaderStyle CssClass="text-center"/>
                    <ItemTemplate>
                        <asp:Label runat="server" ID ="lblOStartDT" Text ='<%#Bind("Expire_SDate","{0:yyyy/MM/dd}") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
                    ...
                    <HeaderStyle  CssClass="text-center"/>
                    <ItemTemplate>
                        <asp:Label runat="server" ID="lblOEndDT" Text='<%#Bind("Expire_EDate","{0:yyyy/MM/dd}") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
