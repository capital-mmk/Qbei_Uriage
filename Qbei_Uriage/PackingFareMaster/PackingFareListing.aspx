<%@ Page Title="" Language="C#" MasterPageFile="~/Uriage.Master" AutoEventWireup="true" CodeBehind="PackingFareListing.aspx.cs" Inherits="Qbei_Uriage.PackingFareMaster.PackingFareListing" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script type="text/javascript">
        $(document).ready(function () {
            $("#<%=txtPUnitPrice.ClientID%>").on("keypress keyup", function (e) {
                $(this).val($(this).val().replace(/[^0-9\.]/g, ''));
                if (e.which < 48 || e.which > 57)
                    e.preventDefault();
            });

            $("#<%=gvPackingFare.ClientID%> [id*=btnDelete]").click(function () {
                ...
                if (confirm("削除よろしいですか？"))
                {
                    $.ajax({
                        ...
                        url: "PackingFareListing.aspx/PackingFareDelete",
                        ...
                        success: function (msg) {
                            ...
                            {
                                temp.remove();
                                if ($("#<%=gvPackingFare.ClientID%> td").length == 0)
                                { }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div class="panel panel-custom1" style="margin-top:-40px;">
        <div class="panel-heading" style="width:100%"><span class="glyphicon glyphicon-file">荷造運賃一覧</span></div>
                ...
                <div class="panel-body" style="padding: 10px 0px 0px 0px; margin-left:10px; margin-top:10px;">
                    <table style="height: 80px; width: 1328px;">
                        <tr>
                            <td>勘定科目</td>
                            <td>
                                <asp:TextBox ID="txtPAcc" runat="server" Width="130px" Height="25px"></asp:TextBox>
                            </td>
                            <td>注文タイプ</td>
                            <td>
                                <asp:DropDownList runat="server" ID="ddlOrderType" Height="30px"  Width="130px">
                                    <asp:ListItem Text="" Value="-1"></asp:ListItem>
                                    <asp:ListItem Text="自転車含む" Value="1"></asp:ListItem>
                                    <asp:ListItem Text="自転車以外" Value="0"></asp:ListItem>
                                </asp:DropDownList>
                            </td>
                            <td>配送会社コード</td>
                            <td>
                                <asp:TextBox ID="txtDeliveryCd" onkeypress="return isNumber(event)" runat="server" Width="130px" Height="25px"></asp:TextBox>
                            </td>
                            <td>配送先県</td>
                            <td>
                                <asp:DropDownList runat="server" ID="ddlRegion" AppendDataBoundItems="true" CssClass="dropdown" Height="30px">
                                    <asp:ListItem Text="" Value="-1"></asp:ListItem>
                                </asp:DropDownList>
                            </td>
                        </tr>
                        <tr>
                            <td>単価</td>
                            <td>
                                <asp:TextBox ID="txtPUnitPrice" runat="server" onkeypress="return isNumber(event)" Width="130px" Height="25px"></asp:TextBox>&nbsp;円
                            </td>
                            <td>開始日付</td>
                            <td>
    ...
    <div align="center">
        <asp:GridView ID="gvPackingFare" runat="server" AllowPaging="true" AutoGenerateColumns="false" Width="98%"
            ShowHeaderWhenEmpty ="true" CssClass ="table table-striped table-bordered table-hover usd header-m" OnPageIndexChanging="gvPackingFare_PageIndexChanging"
            ...
            <Columns>
                <asp:TemplateField>
                    ...
                    <ItemTemplate>
                        <asp:HiddenField runat="server" ID="hdnPackingId" Value='<%#Bind("ID") %>' />
                        <asp:Label runat="server" ID="lblPAcc" Text ='<%#Bind("AccountTitle") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
                    <HeaderStyle CssClass="text-center" />
                    <HeaderTemplate>注文タイプ</HeaderTemplate>
                    <ItemTemplate>
                        <asp:Label runat="server" ID="lblOrder" Text='<%# Eval("OrderType").ToString () == bool.FalseString ? "自転車以外" : "自転車含む" %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
                    <HeaderStyle CssClass="text-center" />
                    <HeaderTemplate>配送会社コード</HeaderTemplate>
                    <ItemTemplate>
                        <asp:Label runat="server" ID="lblDeliveryCd" Text='<%#Bind("DeliveryCompanyCode") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
                    <HeaderStyle CssClass="text-center" />
                    <HeaderTemplate>配送先県</HeaderTemplate>
                    <ItemTemplate>
                        <asp:Label runat="server" ID="lblRegion" Text='<%#Bind("Region") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
                    <HeaderStyle CssClass="text-center" />
                    ...
                    <ItemTemplate>
                        <asp:Label runat="server" ID="lblPUnitPrice" Text='<%#int.Parse (Eval("UnitPrice").ToString()).ToString("#,###") + " 円" %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
                    <HeaderStyle CssClass="text-center" />
                    ...
                    <ItemTemplate>
                        <asp:Label runat="server" ID="lblPStartDT" Text ='<%#Bind("Expire_SDate","{0:yyyy/MM/dd}") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
                    <HeaderStyle CssClass="text-center" />
                    ...
                    <ItemTemplate>
                        <asp:Label runat="server" ID="lblPEndDT" Text='<%#Bind("Expire_EDate","{0:yyyy/MM/dd}") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
